...
###    my $sql = 'select id, parent, entryDate, category, title, text, status from entries order by id desc';
    ...
    $sth->execute or die $sth->errstr;
###    template 'show_entries_block.tt', {
    template 'show_db_entries_block.tt', {
        'msg' => get_flash(),
        'add_entry_url' => uri_for('/adddbblock'),
        'db_entries' => $sth->fetchall_hashref('id'),
    };
};
    ...
    foreach my $item (@array){
        ...
        ###   my($parent,$entryParent,$entryType,$handDate,$entryTitle,$authorName,$textNote) = split(/[;,]\s?/,$clean);
         ...
         ###  chomp($entryParent);
           ...
           ###$sth->execute(params->{'parent'},params->{'category'},params->{'title'}, params->{'text'}) or die $sth->errstr;
           ###$sth->execute($parent,$category,$title,$text) or die $sth->errstr;
           $sth->execute($parent,$entryParent,$entryType,$handDate,$entryTitle,$authorName,$textNote) or die $sth->errstr;
           ###$sth->execute or die $sth->errstr;
 };
    set_flash('New entry posted!');
    redirect '/DBBlock';
